...
export function ProductSection({ title, category, showViewAll = true, maxProducts = 6, isLoading = false }) {
  ...
  return (
    <section className="py-4 sm:py-4 md:py-4 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="py-6 bg-white">
                  ...
                  >
                    ...
                    >
                      <div className="relative aspect-square overflow-hidden mb-3">
                        ...
                        {product.discount > 0 && (
                          <div className="absolute top-1 left-1 flex  items-center gap-x-2">
                            <div className="bg-red-500 w-[80px] text-xs font-bold px-2 py-1 rounded-full shadow-lg animate-pulse">
                              <span className="text-white">{product.discount}% OFF</span>
                            </div>
                            <span className="text-xs text-black font-bold bg-white/80  py-0.5 rounded">{product.saleName}</span>
                          </div>
                        )}
