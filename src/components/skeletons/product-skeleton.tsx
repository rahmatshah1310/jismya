...
export const AllProductSkeleton = ({ count = 12 }) => {
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <Skeleton key={i} className="animate-pulse bg-white rounded-md shadow-sm p-4 flex flex-col">
          {/* Image placeholder */}
          <Skeleton className="bg-gray-300 h-56 w-full mb-4 rounded-md"></Skeleton>

          {/* Title */}
          <Skeleton className="bg-gray-300 h-4 w-3/4 mb-2 rounded"></Skeleton>
          {/* Subtitle */}
          <Skeleton className="bg-gray-300 h-4 w-1/2 mb-2 rounded"></Skeleton>

          {/* Price / button row */}
          <Skeleton className="flex gap-2 mt-auto">
            <Skeleton className="bg-gray-300 h-8 w-16 rounded"></Skeleton>
            <Skeleton className="bg-gray-300 h-8 w-8 rounded"></Skeleton>
          </Skeleton>
        </Skeleton>
      ))}
    </>
