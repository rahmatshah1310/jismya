...
const Header = () => {
  ...
  const { getCartItemCount, getCartTotal, wishlist } = useCart();
  const cartItemCount = getCartItemCount?.() ?? 0;
  const totalPrice = getCartTotal?.() ?? 0;
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/search?item=${encodeURIComponent(searchQuery.trim())}`);
  ...
  const categoryOptions = Array.isArray(categoriesResponse?.data) ? categoriesResponse.data.map((cat: any) => ({ label: cat.name, value: cat._id })) : [];
  const options = [{ label: "All Categories", value: "0" }, ...categoryOptions];
              ...
              <div className="flex items-center gap-5">
                <Link href="/wishlist" className="flex items-center gap-2.5">
                  <span className="inline-block relative">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                        d="M12.0002 21.35L10.55 20.03C5.40002 15.36 2.00024 12.28 2.00024 8.5C2.00024 5.42 4.42024 3 7.50024 3C9.24024 3 10.9102 3.81 12.0002 5.08C13.0902 3.81 14.7602 3 16.5002 3C19.5802 3 22.0002 5.42 22.0002 8.5C22.0002 12.28 18.6004 15.36 13.4504 20.04L12.0002 21.35Z"
                        fill="#3C50E0"
                      />
                    </svg>
                  ...
                  <div>
                    <span className="block text-2xs text-dark-4 uppercase">wishlist</span>
                    <p className="font-medium text-custom-sm text-dark">{wishlistCount}</p>
                  </div>
                </Link>
                ...
                <button onClick={handleOpenCartModal} className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-custom-sm text-dark">Rs.{totalPrice}</p>
                  </div>
                </button>
              </div>
